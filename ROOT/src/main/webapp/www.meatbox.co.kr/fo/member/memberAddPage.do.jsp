...
<script type="text/javascript" src="https://static-cdn.meatbox.co.kr/js/tool/ThumbnailTool.min.js?ver=1760075298515"></script>
...
<script type="text/javascript">
  ...
  (function() {
    var me = window.MainMgr = {
      ...
      setVariables: function() {
        ...
        me.isDuplAccounts = false; //중복계정 유무
        me.isNonKorean = false; //외국인 유무

        //sns 회원가입 시 sns api 에서 받아온 정보
        me.email = ''; //이메일
        me.realName = ''; //이름
        me.siteCd = '';
        me.siteId = '';
      ...
      initialize: function() {
        if(me.type == 'sns'){
          $('#normal').find('span em').text('')
        }
      },

      checkRole: function(role){
    	  // sns 회원가입 관련 로직은 그대로 유지합니다.
    	  if(me.type == 'sns'){
    	    var params = {
    	      'role': role,
    	      'type': me.type,
    	      ...
    	      'authCi' : me.authCi,
    	      'cellNo' : me.cellNo,
    	      'isNonKorean' : me.isNonKorean,
    	    };
    	    SignUpTool.goPersonalInfoPage(params);
    	  } else {
    	    // 일반 회원가입 로직을 요청하신 대로 수정합니다.
    	    if(role == 'B002'){ // ✨ '사업자 회원가입' 버튼을 눌렀을 때
    	      window.location.href = '/registerSeller.do';
    	    } else if(role == 'B004'){ // ✨ '개인 회원가입' 버튼을 눌렀을 때
    	      window.location.href = '/registerNormal.do';
    	    }
    	  }
    	},
      nothing:null
  ...
  function goMemberTerms(param){
    var params = JSON.parse(param);

    SignUpTool.goMemberTermsPage(params); //이용약관 페이지
  }
  ...
  function goMemberType(param){
    SignUpTool.goMemberTypePage(); //가입유형 페이지
  }
  ...
  <div class="content">
    <div id="join_login" class="join_form"><!-- //join_form: 회원가입 대표 클래스 | 영역 나눠짐 -->
      <div class="join-form-comm">
        <div class="join-tit center">
          <h2>No.1 축산물 직거래 플랫폼<br><em class="ce853">미트박스</em>에 오신 것을<br>환영합니다</h2>
          <p class="c8080">가입하실 회원 유형을 선택해주세요.</p>
        </div>
      </div><!-- //join-comm -->
      <div class="form_box">
        <div class="join-type">
          <ul>
            <li>
              <a href="javascript:;" onclick="MainMgr.checkRole('B002');" id="buyer">
                <img src="https://static-cdn.meatbox.co.kr/img/mo/ico/ico-member-type01.png" alt="사업자 회원가입">
                <span><b>사업자 회원가입</b></span>
              </a>
            </li>
            <li>
              <a href="javascript:;" onclick="MainMgr.checkRole('B004');" id="normal">
                <img src="https://static-cdn.meatbox.co.kr/img/mo/ico/ico-member-type02.png" alt="개인 회원가입">
                <span><b>개인 회원가입</b><em>본인인증 후 가입</em></span><!-- ! 개인 회원가입 "본인인증 후 가입" 문구는 일반 회원가입시에만 노출 -->
              </a>
            </li>
          </ul>
        </div><!-- //join-type -->
