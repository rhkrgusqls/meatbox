...
<script type="text/javascript" src="https://static-cdn.meatbox.co.kr/js/tool/ThumbnailTool.min.js?ver=1760075299535"></script>
    ...
    var me = window.HeaderMgr = {
        ...
        const loginSuccessCallback = (result) => {
          ...
          if(info.login){
            ...
            if (info.bo) {
              maxLength = 5;
              namePrefix += '[관리자] ';
              $('#membershipImg').attr('src', me.STATIC_SERVER +'/img/renew/icon-membership-admin.svg');
              $("#membershipImg").show();
            }
            ...
            switch(info.role){
              case 'B002': // 사업자
                nameSuffix = ' 사장님';
                loginName = me.textEllipsis(info.coName, 10);
                membershipName = me.textEllipsis(info.coName, maxLength);
                if (info.meatMartYn === 'Y') {
                  $("#membershipImg").attr("src", me.membershipImg[info.role]).show();
                  $('#gradeTopImg').attr('src',me.membershipImg[info.role]).show();
                  $('#gradeInImage').attr('src',me.gradeImg[info.buyerGradeCd]).show();
                } else {
                  $('.gradeImg').attr('src',me.gradeImg[info.buyerGradeCd]).show();
                }
      ...
      searchInputOnChange: function (searchInputEl, searchDeleteBtnEl) {
        ...
        searchInputEl.addEventListener('input', (e) => {
          ...
          const autoCompleteSuccessCallback = (result) => {
            const data = result.data;
            ...
            $keywordInner.empty();
            if ((autoCompleteKeyword === undefined || autoCompleteKeyword.total === undefined || autoCompleteKeyword.total === 0) &&
      ...
      searchWindowToggle: function (searchAreaEl, searchInputEl, searchDropBoxEl, searchDropBoxCloseBtnEl) {
        searchInputEl.addEventListener('focus', (e) => {
          ...
          const $searchDefaultBox = $(".search-default-box");
          $searchDefaultBox.show();
          ...
          function searchKeywordCallback(result) {
            ...
            $keywordInner.empty();
            searchWordList.forEach((item, index) => {
          ...
          function searchEventsCallback(result) {
            ...
            events.forEach(item => {
              const html = "<li class=\"swiper-slide\" style=\"margin-right: 8px;\">" +
          ...
          FoTool.fetchApiAjaxResult({
            ...
            method: 'GET',
            successCallback: searchKeywordCallback,
            errorCallBack: null
          });
          ...
          FoTool.fetchApiAjaxResult({
            ...
            method: 'GET',
            successCallback: recommendKeywordCallback,
            errorCallBack: null
          });
          ...
          FoTool.fetchApiAjaxResult({
            ...
            method: 'GET',
            successCallback: searchEventsCallback,
            errorCallBack: null
          });
        ...
        document.addEventListener('click', (e) => {
          ...
          } else if(!searchAreaEl.contains(clickEventTarget)) searchDropBoxEl.style.display = 'none';
        });
  ...
  (function() {
    ...
    var me = window.MainMgr = {
      ...
      setVariables: function() {
        //기본 정보 파라미터
        me.role = 'B002'; //사업자, 개인
        me.type = 'default'; //일반 회원가입, sns 회원가입 타입
        me.retry = 'false'; //sns회원가입에서 본인인증PASS 인 경우 true
        ...
        me.$termsCheck	= $("#termsForm input[type='checkbox']");
      ...
      setEvents: function() {
        ...
        $('#chk_all').click(function(){
          $('.essential_area input:checkbox').not(this).prop('checked', this.checked);
        });

        //프로모션 전체 동의
        $('.check_indent #chk_prom').click(function(){
          $(this).closest('.check_indent').find('input:checkbox').not(this).prop('checked', this.checked);
        });

        //프로모션 체크박스 개수 체크
        $('.login_saves input:checkbox').click(function() {
          var promCheckAll = $('.login_saves input:checkbox').length;
          var promCheckedLeng = $('.login_saves input:checkbox').filter(':checked').length;

          $('.check_indent #chk_prom').prop('checked', false);
          if(promCheckedLeng > 0) $('.check_indent #chk_prom').prop('checked', true);
        });

        //전체 체크박스 체크
        $('.essential_area input:checkbox').click(function () {
          var is_checked = true;

          $('.essential_area > .checked_agree label.check input:checkbox').each(function () {
            if (!($(this).closest('div').hasClass('login_saves'))) {
        ...
        //다음 단계 버튼 활성화
        $('.agree_checkbox input:checkbox').change(function(){
          var sntlInputLeng = $('.essential_area label.sntl > input:checkbox').length;
          var sntlcheckedLeng = $('.essential_area label.sntl > input:checkbox').filter(':checked').length;


          if(sntlInputLeng == sntlcheckedLeng) {
            $('.join_form .comm_btn').prop('disabled', false);
          } else {
            $('.join_form .comm_btn').prop('disabled', true);
          }
        });

        //이용약관 동의 팝업 열기 공통 스크립트
        $('.arrow_hidden').click(function() {
          var targetPop = $(this).data('pop');
          $('.member-terms' + '-' + targetPop).addClass('on').wrap('<div class="dim-layer"></div>');

          if($('#chk_buy').is(':checked')){
            $('.member-terms-pop01 .chkeck_che label[name=agree]').addClass('selected');
            $('.member-terms-pop01 .chkeck_che label[name=disagree]').removeClass('selected');
          }else{
            $('.member-terms-pop01 .chkeck_che label[name=agree]').removeClass('selected');
            $('.member-terms-pop01 .chkeck_che label[name=disagree]').addClass('selected');
          }

          if($('#chk_get01').is(':checked')){
            $('.member-terms-pop02 .chkeck_che label[name=agree]').addClass('selected');
            $('.member-terms-pop02 .chkeck_che label[name=disagree]').removeClass('selected');
          }else{
            $('.member-terms-pop02 .chkeck_che label[name=agree]').removeClass('selected');
            $('.member-terms-pop02 .chkeck_che label[name=disagree]').addClass('selected');
          }

          if($('#chk_get02').is(':checked')){
            $('.member-terms-pop03 .chkeck_che label[name=agree]').addClass('selected');
            $('.member-terms-pop03 .chkeck_che label[name=disagree]').removeClass('selected');
          }else{
            $('.member-terms-pop03 .chkeck_che label[name=agree]').removeClass('selected');
            $('.member-terms-pop03 .chkeck_che label[name=disagree]').addClass('selected');
          }

          if($('#chk_prom').is(':checked')){
            $('.member-terms-pop04 .chkeck_che label[name=agree]').addClass('selected');
            $('.member-terms-pop04 .chkeck_che label[name=disagree]').removeClass('selected');
          }else{
            $('.member-terms-pop04 .chkeck_che label[name=agree]').removeClass('selected');
            $('.member-terms-pop04 .chkeck_che label[name=disagree]').addClass('selected');
          }
        });

        //이용약관 동의 팝업 닫기 공통 스크립트
        $('.alert-pops .close').click(function() {
          $(this).closest('.alert-pops').removeClass('on').unwrap('.dim-layer');
        });

        //딤 클릭 시 팝업 닫기 스크립트
        $(document).mouseup(function (e){
          var layerPop = $('.dim-layer');
          var innerPop = layerPop.find('.alert-pops');
          if(layerPop.has(e.target).length === 0){
            innerPop.removeClass('on').unwrap('.dim-layer');
          }
        });

        //팝업 - (필수) 이용약관 동의
        $('#member-terms-pop01_agree').click(function() {
          if(!$('#chk_buy').is(':checked')){
            $('#chk_buy').click();
          }
        });
        $('#member-terms-pop01_disagree').click(function() {
          if($('#chk_buy').is(':checked')){
            $('#chk_buy').click();
          }
        });
        ...
        //(필수) 개인정보 수집 및 이용 동의
        $('#member-terms-pop02_agree').click(function() {
          if(!$('#chk_get01').is(':checked')){
            $('#chk_get01').click();
          }
        });
        $('#member-terms-pop02_disagree').click(function() {
          if($('#chk_get01').is(':checked')){
            $('#chk_get01').click();
          }
        });
        ...
        //(선택) 개인정보 수집 및 이용 동의
        $('#member-terms-pop03_agree').click(function() {
          if(!$('#chk_get02').is(':checked')){
            $('#chk_get02').click();
          }
        });
        $('#member-terms-pop03_disagree').click(function() {
          if($('#chk_get02').is(':checked')){
            $('#chk_get02').click();
          }
        });
        ...
        //(선택) 광고성 정보 수신 동의
        $('#member-terms-pop04_agree').click(function() {
          if(!$('#chk_prom').is(':checked')){
            $('#chk_prom').click();
          }
        });
        $('#member-terms-pop04_disagree').click(function() {
          if($('#chk_prom').is(':checked')){
            $('#chk_prom').click();
          }
        });
      ...
      initialize: function() {
        //개인 회원은 본인인증을 하고 왔는지 체크 필요 -> 회원가입할 때 본인인증 한 데이터들이 필요함
      ...
      checkTerms: function(){
        var params = {
          ...
          'cellNo' 		: me.cellNo,
          'email' 		: me.email,
          'joinPathCd' 	: me.joinPathCd,
          'joinPath' 		: me.joinPath,
        ...
        //약관 파라미터 세팅
        params = me.setTermsParam(params);

        // 사업자 경우 -> 사업자정보 businessInfoPage.jsp
        if(me.role == 'B002'){
          SignUpTool.goBusinessInfoPage(params);
        }else{// 개인 경우 -> 개인정보 personalInfoPage.jsp
          SignUpTool.goPersonalInfoPage(params);
        }
      },

      getTermsParam: function() {
        var termsParam = {};
        me.$termsCheck.each(function(idx,item){
          if ($(this).attr('name') !== '') {
            termsParam[$(this).attr('name')] = $(this).is(':checked') ? 'Y' : 'N';
          }
        });
        return termsParam;
      },

      setTermsParam: function(params){
        var termsParam = me.getTermsParam();
        return $.extend(params,termsParam);
      },
      //최종 찐 회원가입!
      addMember: function(){
        var params = {
          ...
          'office'			: 'fo',
          'role' 				: me.role, //사업자, 개인
          'type'				: me.type, //일반, sns 회원가입
          'retry'				: me.retry, //sns회원가입에서 본인인증PASS 인 경우 true

          //개인 정보 입력 파라미터
          'memberId'			: me.memberId, //아이디
          'passwd'			: me.passwd, //비밀번호
          'realName'			: me.realName, //이름
          'cellNo'			: me.cellNo, //휴대폰번호 -> 개인/사업자 경우 필드 영역이 다르다
          'email'				: me.email, //이메일
          'joinPathCd'		: me.joinPathCd, //가입경로
          'joinPath'			: me.joinPath, //가입경로 - 기타 경우
...
<div class="container white_box">
  <div class="content">
    <div id="join_login" class="join_form"><!-- //join_form: 회원가입 대표 클래스 | 영역 나눠짐 -->
      <div class="join-form-comm">
        <div class="join-tit">
          <h4>사업자 회원가입</h4>
          <h2><em class="ce853">이용약관</em>을 확인하신 후<br>동의해주세요</h2>
        </div>
      </div><!-- //join-form-comm -->
      <div class="form_box">
        <div class="agree_checkbox">
          <form action="" id="termsForm">
            <div class="checkbox_width">
              <div class="chkeck_che">
                <label class="check" for="chk_all">모두 동의
                  <em class="c8080">*선택 정보 포함</em>
                  <input type="checkbox" id="chk_all" name="">
                </label>
              </div>
              <div class="essential_area">
                <div class="chkeck_che checked_agree">
                  <label class="check sntl" for="chk_14"><em class="ce853">(필수)</em> 만 14세 이상입니다.
                    <input type="checkbox" id="chk_14" name="">
                  </label>
                </div><!-- //chkeck_che checked_agree -->
                <div class="chkeck_che checked_agree">
                  <label class="check sntl" for="chk_buy"><em class="ce853">(필수)</em> 이용약관 동의
                    <input type="checkbox" id="chk_buy" name="">
                  </label>
                  <button type="button" class="arrow_hidden" data-pop="pop01"></button>
                </div><!-- //chkeck_che checked_agree -->
                <div class="chkeck_che checked_agree">
                  <label class="check sntl" for="chk_get01"><em class="ce853">(필수)</em> 개인정보 수집 및 이용 동의
                    <input type="checkbox" id="chk_get01" name="">
                  </label>
                  <button class="arrow_hidden" type="button" data-pop="pop02"></button>
                </div><!-- //chkeck_che checked_agree -->
                <div class="check_indent checked_agree">
                  <div class="chkeck_che">
                    <label class="check" for="chk_get02"><em class="ca0a4">(선택)</em> 개인정보 수집 및 이용 동의
                      <input type="checkbox" id="chk_get02" name="">
                    </label>
                    <button class="arrow_hidden" type="button" data-pop="pop03"></button>
                  </div>
                </div><!-- //chkeck_che checked_agree -->
                <div class="check_indent checked_agree">
                  <div class="chkeck_che">
                    <label class="check" for="chk_prom"><em class="ca0a4">(선택)</em> 광고성 정보 수신 동의
                      <input type="checkbox" id="chk_prom" name="">
                    </label>
                    <button class="arrow_hidden" type="button" data-pop="pop04"></button>
                  </div><!-- //chkeck_che -->
                  <div class="login_saves">
										<span class="chkeck_che">
											<label class="check" for="chk_sms">문자
												<input type="checkbox" id="chk_sms" name="smsRecvYn">
											</label>
										</span>
                    <span class="chkeck_che"><!-- promtion_chek-->
											<label class="check" for="chk_email">이메일
												<input type="checkbox" id="chk_email" name="emailRecvYn">
											</label>
										</span>
                  </div><!-- //login_saves -->
                </div><!-- //check_indent -->
              ...
              <div class="join-noti">
                <p>고객은 이용약관 관련 항목에 대한 동의를 거부할 권리가 있으며 동의를 거부할 경우, 사이트 가입 또는 일부 서비스 이용이 제한됩니다.</p>
                <p><b>구매정보, 배송 안내, 주요 정책 안내</b>는 광고성 정보 수신 동의와는 관계없이 발송해 드립니다.</p>
              </div><!-- //join-noti -->
            </div><!-- //checkbox_width -->
          </form>
        </div><!-- //agree_checkbox -->
      ...
      <div class="btn_box">
        <button type="button" class="comm_btn" disabled onclick="MainMgr.checkTerms();">다음 단계로</button>
      </div>
    </div><!-- //join_login -->
  </div><!-- //content -->
</div><!-- //container -->

<!-- 회원가입에서만 사용하는 약관동의 팝업!! -->
<!-- 이용약관 동의(필수) -->
<div class="alert-pops member-terms-pop member-terms-pop01">
  <div class="alert">
    <div class="alert_content">
      <div class="alert-tit">
        <h4>이용약관 동의(필수)</h4>
      </div>
      <div class="of-auto terms-cont">
        <b>제 1장 총칙</b>
        <p>제 1조(목적)<br>
          이 약관은 주식회사 미트박스글로벌(이하 "회사"라 함)이 운영하는 미트박스(http://www.meatbox.co.kr, 모바일 웹/앱; 이하 "미트박스"라 함)에서 제공하는 전자상거래 관련 서비스(이하 "서비스"라 함)를 이용함에 있어
          상품 또는 용역을 거래하는 자 간의 권리, 의무 등 기타 필요사항, 회원과 회사 간의 권리, 의무, 책임사항 및 회원의 서비스 이용절차 등에 관한 사항을 규정함을 목적으로 합니다.
          이 약관은 스마트폰 앱, 제휴 채널 등 다양한 전자적 수단을 통한 전자상거래에도 그 성질에 반하지 않는 한 준용됩니다.
        </p>
        <p>제2조 (용어의 정의)<br>
          ① 이 약관에서 사용하는 용어의 정의는 다음과 같습니다.<br>
          1. 미트박스(http://www.meatbox.co.kr) 자유로운 상품 또는 용역(이하 "상품"이라 함)의 거래를 위하여 회사가 제공하는 실시간 전자상거래 시스템 및 그 운영을 위한 웹사이트 및 모바일 웹/앱을 말합니다.<br>
          2. 이용자: 이 약관에 따라 회사가 제공하는 서비스를 받는 회원 및 비회원을 말합니다.<br>
          3. 회원: 회사에 개인정보를 제공하고, 본 약관에 동의하여 회원 등록 절차를 완료한 이용자를 말합니다. 회원은 다음과 같이 구매회원과 판매회원으로 구분이 됩니다.<br>
          가. 구매회원: 미트박스에서 상품 구매 및 구매와 관련하여 제공되는 서비스를 이용할 수 있는 회원을 말합니다. 구매회원은 개인회원과 사업자회원으로 구분됩니다.<br>
          ■ 개인회원: 만 14세 이상으로, 개인적인 소비를 목적으로 회사의 상품 등을 구매하는 일반 소비자<br>
          ■ 사업자회원: 사업자정보를 제공하여 회원으로 등록한 자로, 사업자의 명의 및 정보로 회사의 상품 등을 구매하여 사업에 활용하는 회원 통해 회원으로 등록하여, 사업자의 명의 및 정보로 회사의 상품 등을 구매하여 사업에 활용하는 회원<br>
          나. 판매회원: 사업자등록증을 소지한 법인 또는 개인사업자가 미트박스에 입점 신청 및 회사의 승인을 거쳐 판매회원으로 등록되며, 회사가 정한 판매회원 이용약관의 적용을 받으면서 미트박스에서 상품 또는 서비스의 판매 및 관련 서비스를 이용할 수 있는 회원을 말합니다.<br>
          4. 비회원: 회원에 가입하지 않고 회사가 제공하는 서비스를 이용하는 자를 말합니다.<br>
          5. 아이디(ID): 회원의 식별과 회원의 서비스 이용을 위하여 회원이 설정하고 회사가 승인을 거쳐 회사에 등록된 영문, 숫자 또는 영문과 숫자의 조합을 말합니다. 아이디는 중복될 수 없으며, 타인의 명의를 도용하여 사용할 수 없고, 회사는 부적절하거나 법령에 위반되는 아이디에 대해서는 등록을 거부하거나 삭제할 수 있습니다.<br>
          6. 비밀번호: 회원의 동일성 확인과 비밀보호를 위하여 회원 스스로가 설정하여 회사에 등록한 영문, 숫자 및 특수문자를 포함한 조합을 말합니다. 비밀번호는 보안 강화를 위해 일정 길이 이상이어야 하며, 회사는 보안상 필요한 경우 비밀번호 변경을 요청하거나 부적절한 비밀번호에 대해 등록을 거부할 권한이 있습니다.<br>
          7. 운영자: 회사가 제공하는 서비스의 전반적인 관리 및 원활한 운영을 위하여 회사가 선정한 자를 말하며, 서비스의 관리, 점검, 회원 지원, 시스템 유지 및 기타 운영 관련 업무를 수행합니다. 운영자는 회원과의 직접적인 거래 당사자가 아니며, 회사의 업무 수행을 위한 대리인으로서 제한된 권한을 가집니다.<br>
          8. 구매자: 회사의 서비스 내에서 판매회원이 등록한 상품을 구매하는 자를 말하며, 개인회원 및 사업자회원 모두 구매자가 될 수 있습니다. 구매자는 회사의 회원 자격 요건을 충족하여야 하며, 상품 구매 시 회사 및 판매자와의 거래에 대한 계약상 권리와 의무를 부담합니다.<br>
          9. 판매자: 상품의 판매를 목적으로 회사가 제공하는 웹사이트 및 서비스 내 상품 등록 양식에 따라 상품을 등록하고 판매 활동을 하는 회원을 말합니다. 판매자는 사업자등록증을 보유한 법인 또는 개인사업자로서 회사의 승인을 받아 판매회원으로 가입한 자가 해당하며, 판매와 관련하여 발생하는 거래 책임과 의무를 집니다.<br>
          10. 회원등급: 회사가 제공하는 서비스를 이용한 거래 실적, 활동 내역 및 기타 회사가 정한 기준에 따라 부여하는 등급을 말하며, 본 등급은 오직 사업자 회원에게만 부여됩니다. 회원등급은 서비스 이용과 관련한 다양한 혜택의 적용 기준으로 활용되며, 회사는 내부 정책 및 이용 실적에 따라 등급을 변경할 수 있습니다.<br>
          11. 상품평(후기): 상품 구매 후 해당 상품에 대해 실제 구매자만 작성할 수 있는 종합적인 평가의 글, 이미지 등의 게재물을 말합니다. 상품평은 구매자 본인의 경험과 의견에 근거하여 작성되어야 하며, 회사는 허위, 비방, 명예훼손 등 부적절한 내용에 대해 삭제, 수정 또는 제한할 수 있습니다.<br>
          12. 일반예치금: 회원이 미트박스에서 상품 구매 및 관련 서비스 이용 중 거래 과정에서 발생한 금액이 누적되어 보관되는 선불 전자지급결제수단을 말합니다. 일반예치금은 거래정산, 환불, 차감 등 회사가 정한 기준과 절차에 따라 자동 처리되며, 거래 내역에 따라 결제 금액이 부족한 경우 마이너스 예치금이 발생할 수 있습니다. 구매회원의 마이너스 예치금은 이후 거래 등을 통해 상환되어야 하며, 회사는 부정 사용 방지 및 관리 차원에서 관련 조치를 할 수 있습니다.<br>
          13. 상품권: 미트박스에서 판매하는 선불식 모바일 상품권으로, 구매 후 회사의 시스템에서 충전 예치금으로 전환하여 미트박스 내에서 결제 수단으로 사용할 수 있는 전자 상품권을 말합니다. 상품권은 회사가 정한 유효기간, 사용 조건, 환불 및 소멸 정책에 따르며, 회원 간 선물 및 양도가 가능하나, 해당 규정을 준수해야 합니다.<br>
          14. 상품권 예치금: 회원이 직접 유상 결제하거나 타 회원으로부터 받은 상품권을 회사 시스템에서 예치금으로 전환하는 등 회사가 정한 다양한 방법으로 충전하여 미트박스 내에서 사용할 수 있는 전자적 지급수단을 말합니다. 상품권 예치금은 일반예치금과 구분되어 별도로 관리되며, 충전, 사용 조건, 유효기간, 환불, 소멸 등은 회사 정책에 따릅니다. 회원은 상품권 예치금 사용 시 회사 규정을 준수해야 하며, 부정 사용 혹은 규정 위반 시 회사는 예치금 회수 및 서비스 이용 제한 조치를 할 수 있습니다.<br>
          15. 미트포인트: 회사가 제공하는 지정된 상품 또는 서비스를 구매할 때 사용할 수 있는 결제수단으로, 특별 이벤트나 프로모션을 통해 구매 금액의 일정 비율이 적립되거나 이벤트 참여를 통해 부여되는 마일리지입니다. 적립된 포인트는 회사가 정한 정책에 따라 사용 및 관리됩니다.<br>
          16. 구매확정기간: 구매자가 거래에 대한 종료 의사표시(구매확정, 반품, 교환)를 해야 하는 기간을 의미하며, 원칙적으로 물품 수령일로부터 3일 이내입니다. 다만, 물품 수령일을 확인할 수 없는 경우에는 발송 완료일로부터 7일이 경과한 후 회사가 상품 수령 확인을 요청한 날로부터 3일 이내로 합니다.<br>
          17. 본인확인 : 개인회원은 회원가입 시 휴대폰 본인인증 서비스(PASS) 또는 네이버, 카카오 등 제휴된 SNS 계정을 통해 본인 여부를 확인할 수 있으며, 회사는 이를 통한 인증정보를 활용하여 회원의 신원을 확인합니다. 사업자 회원은 상호명 및 사업자등록번호의 일치 여부와 제7조에서 정한 기재항목을 확인하는 절차를 거칩니다. 본 약관에서는 i-PIN 인증은 제공하지 않으며, 본인확인 수단은 현재 제공하는 방법으로 제한됩니다.<br>
          18. 영업일: 회사가 정상적으로 서비스를 제공한 날로서 토요일, 일요일 및 법정 공휴일을 제외한 날을 말합니다.<br>
          19. 결제대금 보호서비스: 회사는 구매자가 상품 대금을 결제한 후 일정 기간 동안 해당 대금을 안전하게 예치하며, 구매자가 상품을 정상적으로 수령하거나 거래가 완료된 경우에 한해 판매자에게 대금을 지급하는 서비스를 제공합니다. 본 서비스는 거래의 안전성과 신뢰성 확보를 목적으로 하며, 회사는 결제대금의 보관 및 지급과 관련하여 관련 법령 및 약관에 따라 업무를 수행합니다.<br>
          20. 분쟁조정센터: 원칙적으로 미트박스를 통한 거래와 관련한 분쟁은 판매자와 구매자 간의 자율적 합의로 해결하는 것을 기본으로 합니다. 다만, 합리적이고 원활한 분쟁 해결을 돕기 위하여 회사는 분쟁조정센터를 설치·운영하며, 해당 센터는 판매자와 구매자 간의 분쟁 조정을 지원하는 기구입니다.<br>
          ② 제1항에서 정의되지 아니한 이 약관상의 용어의 의미는 일반적인 거래관행에 의합니다.<br>
        </p>
        <p>제3조 (약관의 명시, 효력 및 변경)<br>
          ① 회사는 정의된 약관을 이용자가 알 수 있도록 회사의 서비스가 제공되는 인터넷 사이트 (http://www.meatbox.co.kr)의 첫 화면에 게시합니다.<br>
          ② 회사는 합리적인 사유 발생시 약관의 규제에 관한 법률, 전자거래기본법, 전자서명법, 정보통신망 이용촉진 및 정보보호 등에 관한 법률, 전자상거래 등에서의 소비자보호에 관한 법률 등 관련 법령에 위배되지 않는 범위 안에서 이 약관을 변경 할 수 있으며, 회사가 약관을 변경하는 경우에는 적용일자 및 변경사유를 명시하여 그 적용일자 7일 전부터 미트박스 서비스 사이트에 공지합니다. 다만, 이용자에게 불리한 약관의 변경의 경우에는 그 적용일자 30일 전부터 공지합니다.<br>
          ③ 이 약관에 동의하는 것은 정기적으로 회사가 운영하는 인터넷 사이트를 방문하여 약관의 변경사항을 확인하는 것에 동의함을 의미합니다. 변경된 약관에 대한 정보를 알지 못하여 발생하는 회원 또는 이용자의 피해에 대하여 회사는 책임을 부담하지 않습니다.<br>
          ④ 회원은 변경된 약관에 동의하지 않을 경우 회원 탈퇴(이용계약의 해지)를 요청할 수 있으며, 회사가 제2항에 따라 변경된 약관을 공지 또는 통지하면서 이용자에게 약관 변경 적용일까지 거부의사를 표시하지 아니할 경우, 약관의 변경에 동의한 것으로 간주한다는 내용을 공지 또는 통지하였음에도 이용자가 명시적으로 약관 변경에 대한 거부의사를 표시하지 아니하면, 회사는 이용자가 변경 약관에 동의한 것으로 간주합니다.<br>
        </p>
        <p>제4조 (약관 외 준칙)<br>
          ① 이 약관에 명시되지 않은 사항은 전자상거래 등에서의 소비자보호에 관한 법률 등 관련 법령의 규정과 일반 상관례에 의합니다.<br>
          ② 회사는 필요한 경우 특정 서비스에 관하여 적용될 사항(이하 “개별약관”이라 함)을 정하여 이를 미트박스 등을 통하여 공지할 수 있습니다.<br>
          ③ 회사는 필요한 경우 서비스 이용과 관련된 세부적인 내용(이하 "이용정책"이라 함)을 정하여 이를 미트박스 등을 통하여 공지할 수 있습니다.<br>
          ④ 개별약관이나 이용정책의 변경이 있을 경우 회사는 변경 내용의 효력발생일 7일 이전에 해당 변경 사항을 공지합니다.<br>
          ⑤ 회원은 항상 개별약관이나 이용정책의 내용에 변경이 있는지 여부를 주시하여야 하며, 변경사항의 공지가 있을 시에는 이를 확인하여야 합니다.<br>
        </p>
        <p>제5조 (이용계약의 성립)<br>
          ① 회사가 제공하는 서비스에 관한 이용계약(이하 "이용계약"이라고 합니다)은 서비스를 이용하고자 하는 자의 이용신청에 대하여 회사가 승낙함으로써 성립하며, 서비스를 이용하고자 하는 자는 회사가 온라인으로 제공하는 회원가입 신청 양식에 따라 필요한 각각의 항목을 기입한 후 이 약관에 동의한다는 의사 표시를 함으로써 이용신청을 합니다.<br>
          ② 회원가입의 성립시기는 회사가 이용신청에 대한 승낙 의사를 해당 서비스 화면에 게시하거나 기타 회사가 정한 방법으로 이용신청자에게 전달한 시점으로 합니다. 이용신청자는 화면 안내를 확인함으로써 회원가입이 성립된 것으로 간주됩니다.<br>
          ③ 회원가입은 개인의 경우 만 14세 이상의 자면 신청할 수 있으며, 사업자(개인사업자 및 법인사업자 포함)의 경우는 가입 신청일 현재 정상적인 영업활동이 가능한 사업자이면 신청을 할 수 있습니다.<br>
          ④ 회원ID는 개인회원의 경우, 본인 인증 시 부여되는 고유 식별정보(CI)를 기준으로 1인당 1개의 아이디만 사용하는 것을 원칙으로 합니다. 사업자회원의 경우에는 사업자등록번호를 기준으로 1개의 아이디만 사용할 수 있습니다. 회사는 동일 CI 또는 사업자등록번호로 복수 가입이 확인될 경우, 이중 가입 방지 및 서비스 운영의 효율성을 위해 해당 회원ID의 이용을 제한하거나 통합 조치할 수 있습니다.<br>
          ⑤ 회원이 이용계약을 종료(회원 탈퇴, 사업자의 휴·폐업에 따른 자동 해지 포함)한 후 재가입하는 경우, 기존에 사용하였던 아이디는 사용할 수 없으며 새로운 아이디를 사용하는 것을 원칙으로 합니다. 회사는 재가입 시 부정 가입 방지 및 서비스 운영의 효율성을 위하여 이전 이용 기록을 참고하여 필요 시 재가입 승낙을 제한할 수 있습니다.<br>
          ⑥ 개인회원은 관련 법령에 따라 신뢰할 수 있는 본인확인기관(휴대폰 본인확인 서비스인 PASS 인증)을 통해 본인 여부를 확인하는 본인확인 절차를 거쳐야 합니다. 회사는 본인확인 절차를 진행할 수 없는 이용신청에 대해 신청자에게 추가적인 증빙자료를 요청할 수 있으며, 신청자가 이를 제출하지 않을 경우 이용승낙을 거부할 수 있습니다.<br>
          ⑦ 회원이 자발적으로 탈퇴한 후에는 1개월 이내에 재가입할 수 없으며, 이는 회사가 제공하는 할인쿠폰, 이벤트 혜택 등 경제적 이익의 부정 수취 및 명의 도용 등의 부정 행위 우려를 방지하기 위한 조치입니다. 회사는 재가입 제한 기간 내에 신청하는 경우 서비스 이용계약 체결을 거부할 수 있습니다.<br>
        </p>
        <p>제6조 (회원정보의 변경, 보호)<br>
          ① 이 서비스에 가입하여 이용하고자 하는 자는 회사가 정한 가입 절차에 따라 회원 아이디(ID)를 직접 설정하여 등록하거나, 네이버, 카카오 등 제휴된 SNS 로그인을 통해 자동 생성된 아이디를 사용하여 회원가입 절차를 진행합니다. 회원가입은 회사가 정한 방법 및 절차에 따라 이루어지며, 이에 따라 회원 자격이 부여됩니다.<br>
          ② 회원은 이용신청 시 기재한 사항이 변경되었을 경우에는 즉시 변경사항을 최신의 정보로 수정하여야 합니다. 단, 이름, ID 등 변경이 불가능한 사항은 수정할 수 없습니다.<br>
          ③ 수정하지 않은 정보로 인하여 발생하는 손해는 해당 회원이 부담하며, 회사는 이에 대하여 아무런 책임을 지지 않습니다.<br>
          ④ 회사는 이용자의 정보를 서비스 제공을 위한 목적 이외의 용도로 사용하거나 이용자의 동의 없이 제3자에게 제공하지 않습니다. 단, 다음의 경우는 예외로 합니다.<br>
          ■ 법령이 회원의 개인정보의 이용과 제3자에 대한 정보제공을 허용하고 있는 경우<br>
          ■ 판매자 또는 배송업체 등에게 거래 및 배송 등에 필요한 최소한의 이용자정보(성명, 주소, 연락처)를 제공하는 경우<br>
          ■ 부가가치세법 제 32조에 따른 전자(세금)계산서 발행에 필요한 최소한의 이용자정보((세금)계산서발행정보)를 제공하는 경우<br>
          ■ 구매가 성사된 때에 그 이행을 위하여 필요한 경우와 구매가 종료된 이후에도 반품, 교환, 환불, 취소 등을 위하여 필요한 경우. 다만, 이 경우 회사는 해당 구매의 판매자와 구매자에 한하여 상대방의 연락처 정보를 제공합니다.<br>
          ⑤ 회사는 관련법령이 정하는 바에 따라서 이용자의 개인정보를 보호하기 위하여 노력하며, 이용자의 개인정보를 보호하기 위한 정책을 수립하고 개인정보보호 책임자를 지정하여 이를 게시합니다.<br>
          ⑥ 이용자의 개인정보보호에 관해서는 관련법령 및 회사가 정하는 개인정보 처리방침에서 정한 바에 따릅니다.<br>
        <p>제7조 (서비스 이용 신청)<br>
          ① 회사가 제공하는 서비스에 가입을 하여 이용하고자 하는 자는 회사에서 제공된 회원가입 신청양식에 따라 필요사항을 기재하여야 합니다. 회원가입 시 필수 기재 항목과 선택 기재 항목으로 구분 되어 있으며 필수 기재 항목의 경우 반드시 입력을 해야 합니다. 회원가입 신청양식의 각 필수 항목은 이용신청자가 개인회원인지 사업자회원인지에 따라 다르게 적용됩니다.<br>
          ② 개인회원 필수 기재 항목 : 회원아이디 (ID), 비밀번호, 이름, 휴대전화번호, 이메일주소<br>
          ③ 사업자회원 필수 기재 항목 : 사업자등록번호, 상호, 대표자명, 업태, 업종, 사업장전화번호, 사업장소재지, 사업장 형태(식당, 유통,정육점,기타 등), 아이디(ID), 비밀번호, 이름, 휴대전화번호, 이메일주소<br>
          ④ 회사는 회원 가입 시 제1항 또는 제2항에 표시된 필수항목 외에 추가적인 정보(예시 : 생일, 결혼 여부, 직업, 취미, 관심분야 등)를 요청할 수 있으며, 이용신청자는 그러한 추가적인 정보의 제공을 거절할 수 있습니다. 추가적인 정보는 회원에게 다양하고 유익한 정보를 제공하기 위하여만 사용되며 추가적인 목적으로 사용 할 수가 없습니다.<br>
        </p>
        <p>제8조 (이용신청의 승낙, 거부, 유보)<br>
          ① 회사는 이 약관 제7조에서 정한 기재항목을 사실대로 정확하게 기재하고 이 약관에 동의한 자로서 본인확인절차를 거쳐 확인 된 경우에만 회사 서비스의 이용을 승낙하는 것을 원칙으로 합니다.<br>
          ② 회사는 개인회원 가입신청의 경우 관련 법령에 따라 신뢰할 수 있는 본인확인기관(휴대폰 본인확인 서비스인 PASS 인증)을 통해 본인 여부를 확인하는 본인확인절차를 취하며, 사업자회원의 경우에는 제7조 제4항에 따라 제공받은 증빙자료 등에 의하여 본인확인절차를 취합니다. 회사는 본인확인절차를 취할 수 없는 이용신청에 대해서는 신청자에게 증빙자료를 요청할 수 있습니다.<br>
          ③ 회사는 다음과 같은 사유가 있는 경우, 이용신청에 대한 승낙을 거부할 수 있습니다.<br>
          1. 만 14세 미만의 자가 이용신청을 하는 경우<br>
          2. 회사의 본인확인절차에서 본인이 아님이 확인된 경우<br>
          3. 이미 가입된 회원과 이름 및 CI(Connecting Information, 연계정보, 서비스 연계를 위해 본인확인기관에서 부여하는 개인식별정보), DI(Duplication Information, 중복가입 확인정보) (또는 사업자등록번호, 법인등록번호)가 동일한 경우<br>
          4. 신청, 등록 내용에 허위, 기재누락, 오기 등이 있는 경우<br>
          5. 이용계약이 해지된 날로부터 1개월 이내에 재이용신청을 하는 경우<br>
          6. 회사로부터 회원자격정지 조치 등을 받은 회원이 그 조치기간 중에 이용계약을 임의 해지하고 재이용신청을 하는 경우<br>
          7. 기타 이 약관에 위배되거나 위법 또는 부당한 이용신청임이 확인된 경우<br>
          8. 미트박스 구매회원/판매회원 이용약관의 적용을 받는 자가 해당 약관을 위반한 경우<br>
          ④ 회사는 다음과 같은 사유가 있는 경우, 이용신청에 대한 승낙을 유보할 수 있습니다. 이 경우, 회사는 이용신청자에게 승낙유보의 사유, 승낙가능시기 또는 승낙에 필요한 추가요청정보 내지 자료 등 기타 승낙 유보와 관련된 사항을 해당 서비스화면에 게시하거나 email을 통하여 통지합니다.<br>
          1. 기술상 지장이 있는 경우<br>
          2. 회사의 본인확인절차에서 본인 여부가 확인되지 않는 경우<br>
          3. 기타 회사가 합리적인 판단에 의하여 필요하다고 인정하는 경우<br>
        </p>
        <p>제9조 (이용계약의 종료)<br>
          ① 회원은 다음 각호에서 정한 바에 따라 이용계약을 해지할 수 있습니다.<br>
          1. 회원은 언제든지 회사에 해지의 의사를 통지함으로써 이용계약을 해지할 수 있습니다. 단, 해지의사를 통지하기 전에 현재 진행 중인 모든 상품의 거래를 완료하거나 철회 또는 취소하여야 하며, 거래의 철회 또는 취소로 인한 불이익은 회원 본인이 부담하여야 합니다. 또한, 회원이 회사에 대한 채무를 전부 이행(마이너스 포인트, 마이너스 마일리지의 상환 등)하지 아니한 경우 회원이 당해 채무를 전부 이행할 때까지, 회원이 이 약관 및 관련법령 등을 위반하여 이용이 정지된 경우 회사가 재발방지를 위하여 필요한 경우 회사는 회원의 이용계약 해지를 제한할 수 있습니다.<br>
          2. 회원탈퇴로 인해 발생한 불이익에 대한 책임은 회원 본인이 져야 하며, 이용계약이 종료되면 회사는 회원에게 부가적으로 제공한 각종 혜택을 회수할 수 있습니다.<br>
          3. 회원의 의사로 해지를 한 후 재이용을 희망할 경우에는 회원 재가입, 임의해지 등을 반복적으로 행함으로써 회사가 제공하는 할인쿠폰, 이벤트 혜택 등의 경제상의 이익을 불·편법으로 수취하거나 이 과정에서 명의도용 등의 행위 우려가 있으므로 이러한 불·편법 행위 차단 등의 목적으로 회원탈퇴한 날로부터 1개월이 경과해야 회원 재가입이 가능함을 원칙으로 하며 이 약관이 정하는 회원가입절차와 관련 조항에 따라 재이용 의사가 회사에 통지되어 회사가 이용을 승낙한 이후에 서비스의 재이용이 가능합니다.<br>
          4. 회원 재가입, 임의해지 등을 반복적으로 행함으로써 회사가 제공하는 할인쿠폰, 이벤트 혜택 등의 경제상의 이익을 불·편법으로 수취하거나 이 과정에서 명의도용 등의 행위 우려가 있으므로 이러한 불·편법 행위 차단 등의 목적으로 회원탈퇴 후 1개월 동안은 해당 회원의 아이디, 성명, CI, DI , 탈퇴일자, 탈퇴사유 등 개인정보를 보관합니다.<br>
          ② 회사는 다음 각호에서 정한 바에 따라 이용계약을 해지할 수 있습니다.<br>
          1. 회사는 회원에게 다음 각목의 1에 해당하는 사유가 발생하거나 확인된 경우 이용계약을 해지할 수 있습니다.<br>
          가. 다른 회원·회사(직원·상담원 포함)의 권리나 명예, 신용 기타 정당한 이익을 침해하거나 대한민국 법령 또는 공서양속에 위배되는 행위를 한 경우<br>
          나. 서비스의 원활한 진행을 방해하거나 방해할 우려가 있는 아래의 행위 등을 하거나 시도한 경우<br>
          ■ 합리적 사유 없이 상습적·악의적으로 클레임을 제기하는 행위<br>
          ■ 구매한 상품 또는 서비스에 특별한 하자가 없음에도 불구하고, 상습적으로 사용 후 취소, 반품 등을 하는 행위<br>
          다. 회원의 누적 신용도가 일정 점수 이하에 이른 경우<br>
          라. 고의로 회사의 영업을 방해 하거나 판매부적합 상품을 등록 할 경우<br>
          마. 회원이 이 약관에 위배되는 행위를 하거나 이 약관에서 정한 해지사유가 발생한 경우(미트박스 판매회원 약관의 적용을 받는 자가 해당 약관을 위반한 경우 포함)<br>
          바. 회원에게 제8조에서 정한 이용계약의 승낙거부사유가 있음이 확인된 경우<br>
          사. 기타 회사가 합리적인 판단에 기하여 서비스의 제공을 거부할 필요가 있다고 인정할 경우<br>
          2.회사가 해지를 하는 경우 회사는 회원에게 e-mail, 전화, 팩스, 기타의 방법을 통하여 해지사유를 밝혀 해지의사를 통지합니다. 이용계약은 회사의 해지의사를 회원에게 통지한 시점에 종료됩니다.<br>
          3. 본 항에 의하여 회사가 이용계약을 해지하더라도, 해지 이전에 이미 체결된 매매계약의 완결에 관해서는 이 약관이 계속 적용됩니다.<br>
          4. 본 항에서 정한 바에 따라 이용계약이 종료될 시에는 회사는 별도 통지 없이 해당 회원과 관련된 광고 및 거래를 취소할 수 있고, 회원에게 부가적으로 제공한 제공한 각종 혜택을 회수할 수 있습니다.<br>
          5. 본 항에서 정한 바에 따라 이용계약이 종료되는 경우 회원의 재이용 신청에 대하여 회사는 그 승낙을 거절할 수 있습니다.<br>
          6. 이용계약의 종료와 관련하여 발생한 손해는 이용계약이 종료된 해당 회원이 책임을 부담하여야 하고, 회사는 일체의 책임을 지지 않습니다.<br>
        </p>
        <p>제10조 (회원등급 및 평가제도)<br>
          ① 회사는 서비스를 이용하여 상품을 거래한 경우에 부과되는 점수를 기준으로 하여 회원등급 및 등급에 따른 일정 혜택 등을 부여할 수 있습니다. 회원등급 및 그에 따른 혜택 등에 대하여는 서비스 이용실적, 기타 회사가 정하는 이용정책에 정한 기준에 따라 부여되며, 상세한 내역은 해당 서비스 화면에 게재하거나 회사가 운영하는 인터넷 사이트 초기화면에 공지합니다(회원등급 정책 및 그에 따른 혜택 등에 대한 변경이 있는 경우 이 약관 제3조 제4항에 따라 변경 공지합니다).<br>
          ② 해당 회원이 자신에게 부여된 등급 구성요소에 대하여 이의를 제기하는 경우 회사는 소명내용, 해당 회원의 신용도 등 제반 상황을 감안하여 등급 구성요소의 전부 또는 일부를 조정할 수 있습니다.<br>
          ③ 회원의 등급은 상품매매시의 참고자료로 활용될 뿐이며, 회원의 신용을 보증하거나 금융상의 신용상태를 의미하는 것은 아닙니다.<br>
          ④ 상품의 구매자는 구매확정시에 해당 거래에 대한 만족도를 회사가 정한 단계별로 표시할 수 있으며, 표시된 만족도는 해당 상품 판매자의 평가에 반영됩니다. 거래만족도 항목 및 단계는 해당 서비스 화면에 표시합니다.<br>
          ⑤ 상품평은 구매자를 포함한 모든 회원이 작성할 수 있으며, 공개를 원칙으로 합니다. 상품평 작성자는 자신이 작성한 상품평을 삭제할 수 있습니다. 다만 공개된 상품평 및 첨부된 의견이 공개 부적절하다고 판단한 경우, 회사는 당해 상품평 및 첨부된 의견에 대하여 수정, 삭제할 수 있습니다. 상품평이 삭제되는 경우 상품평에 첨부된 의견도 함께 삭제됩니다.<br>
          ⑥ 거래 상대방에 대한 거래만족도 평가 및 상품평은 전적으로 평가 당사자의 책임 하에 이루어지며, 회사는 내용의 진위 여부에 대해 개입하지 않습니다.<br>
          ⑦ 회원등급 및 회원평가제도의 목적과 취지에 위반하는 행위에 대하여 회사는 해당 평가결과의 삭제 및 관련 회원에 대한 서비스 이용자격의 정지 조치 등의 제재를 가할 수 있습니다.<br>
          ⑧ 아래와 같은 내용에 따라 상품평을 삭제하거나 이용자격이 정지될 수 있습니다.<br>
          1. 회원등급 또는 거래만족도의 증가를 조작하기 위하여 발생하는 만족도 평가행위<br>
          2. 높은 만족도 및 상품평을 쓰는 대가로 금전 또는 기타의 보상을 받거나 받기로 약정하고 이루어진 평가결과 및 그 평가행위<br>
          3. 상품평을 통하여 타인에 대한 모욕, 명예훼손, 신용을 훼손하는 등의 행위 및 그 평가결과<br>
          4. 기타 회원평가제도의 목적과 취지에 반하여 평가제도를 이용하는 행위<br>
        </p>
        <p>제11조 (회원관리)<br>
          ① 회사는 이 약관, 관련 법령 및 상거래의 일반원칙을 위반한 회원에 대하여 다음과 같은 조치를 할 수 있습니다.<br>
          1. 신용점수 차감<br>
          2. 회사가 부가적으로 제공한 혜택의 일부 또는 전부의 회수<br>
          3. 특정 서비스 이용제한<br>
          4. 이용계약의 해지<br>
          5. 손해배상의 청구<br>
          ② 회사가 제1항 각호에 정한 조치를 할 경우 회사는 사전에 회원에게 유선 또는 e-mail로 통보하며, 회원의 연락이 두절되거나 긴급을 요하는 것과 같이 부득이한 경우 선 조치 후 사후 통보할 수 있습니다. 단, 신용점수 차감은 통보 없이 할 수 있습니다.<br>
          ③ 회원은 본 조에 의한 회사의 조치에 대하여 항변의 사유가 있는 경우 이에 대하여 항변을 할 수 있습니다.<br>
        </p>
        <p>제12조 (서비스 기간과 중단)<br>
          ① 회원의 서비스 기간은 미트박스에 서비스를 신청하여 이용승낙이 있은 날로부터 서비스 이용계약의 해지시까지입니다.<br>
          ② 회사는 컴퓨터 등 정보통신설비의 보수, 점검, 교체 및 고장, 통신의 두절 등의 사유가 발생한 경우에는 서비스의 제공을 일시적으로 중단할 수 있습니다. 이 경우 서비스 일시 중단 사실과 그 사유를 사이트 초기화면에 통지합니다.<br>
          ③ 회사는 천재지변 또는 이에 준하는 불가항력으로 인하여 서비스를 제공할 수 없는 경우에는 서비스의 제공을 제한하거나 일시 중단할 수 있습니다<br>
        </p>
        <p>제13조 (서비스 이용료)<br>
          ① 회사는 미트박스의 일부 특정 서비스 또는 기능을 제공하는 데 대해 이용료를 부과할 수 있습니다.<br>
          ② 본 조의 이용료에 관하여는 본 조 및 개별 약관 또는 별도 신청서에서 정하는 바에 따릅니다.<br>
        </p>
        <p>제14조 (할인 쿠폰)<br>
          ① 회사 또는 판매자는 구매서비스를 이용하는 회원에게 상품 구매시 일정금액 또는 일정비율을 할인 받을 수 있는 할인 쿠폰을 발급할 수 있습니다.<br>
          ② 회원은 할인쿠폰을 회원 본인의 구매에 사용할 수 있는 권한만을 가지며, 어떠한 경우에도 이를 타인에게 실질적으로 매매 또는 양도할 수 없습니다.<br>
          ③ 할인쿠폰은 일부 품목이나 금액에 따라 사용이 제한될 수 있으며 유효기간이 지난 후에는 사용할 수 없고, 상품구입 후 취소나 반품으로 인하여 환불이 이루어진 경우에는 재사용이 불가능할 수 있습니다.<br>
          ④ 구매자에게 제공 된 할인쿠폰은 한 상품 구매시 중복으로 사용 할 수가 없습니다. 단, 할인쿠폰의 발행자가 서로 다른 경우(회사 또는 판매자)는 중복으로 사용이 가능한 경우도 있습니다.<br>
          ⑤ 회사는 할인 쿠폰을 발급함에 있어 사용 방법, 사용 기간, 사용 조건, 할인 금액(또는 할인율) 등을 별도로 표시하거나 통지할 수 있습니다. 또한 알림톡, 문자, 이메일 등의 수단을 통해 쿠폰 지급 및 만료 안내를 통지할 수 있습니다.<br>
        </p>
        <p>제15조 (예치금)<br>
          예치금은 미트박스에서 상품 구매의 주문결제 및 대금지불, 회사에서 제공하는 서비스 이용하기 위하여 회원이 신용카드 결제,가상계좌 또는 무통장 입금등의 결제수단을 통하여 구매하는 전자지급수단으로 다음 각호의 규정을 적용 받습니다.<br>
          ■ 예치금은 미트박스에서 사용되는 지불수 단으로 사용 할 수 있으며 주문결제, 대금지불이 가능하며, 1CASH는 1원의 의미를 갖습니다.<br>
          ■ 미트박스에서 회원이 예치금을 통하여 결제를 하여 그 후 상품 구매 취소 등의 사유로 회사가 회원에게 결제 금액을 환불하여야 하나, 그 결제수단의 특성상 동일한 프로세스로 환불이 어려울 경우 회사가 회원에게 환불수단으로 환불금액 1원은 1캐시로 부여하며, 캐시는 현금출금과 포인트로의 전환만 가능하며, 캐시 1원은 현금 1원 또는 1포인트의 의미를 갖습니다.<br>
          ■ 예치금은 현금으로 환급이 가능하며 충전 및 인출의 최소 단위는 1원입니다. 인출 요청 후 2영업일에 지정한 계좌로 환급이 됩니다.<br>
          ■ 예치금에 대해서는 이자가 발생하지 않습니다.<br>
          ■ 환급시 회사는 회원에게 환급 계좌 관련 필요한 자료의 제공을 요구할수 있으며, 회원은 이에 협조하여야 합니다.<br>
          ■ 상품권 예치금은 미트박스 상품권을 구매 후 사용 또는 선물받은 상품권을 수락하여 전환된 예치금을 의미합니다.<br>
          ■ 전환된 상품권 예치금의 유효기간은 전환 된 날로부터 5년입니다.<br>
          ■ 전한된 상품권 예치금의 총합이 1만원 이하는 80% 이상 사용 시, 1만원 초과는 60% 이상 사용 시에만 출금이 가능합니다.<br>
        </p>
        <p>제16조 (상품권)<br>
          미트박스 상품권은 권면에 기재된 금액에 상응하는 상품권 예치금으로 전환할 수 있는 금액 상품권으로서 다음 각 호의 규정을 적용 받습니다.<br>
          ① 상품권의 1회 구매 한도는 200만원으로, 현금성 결제수단으로 구매가 가능합니다.<br>
          ② 미트박스 플랫폼에서 상품권을 사용하여 상품권 예치금으로 전환 후 미트박스 플랫폼에서 자유롭게 사용하거나 상품권을 타인에게 선물할 수 있습니다. 단, 상품권을 선물한 경우에는 선물받은자가 7일 이내에 수락하지 않거나, 선물을 거절한 경우에는 결제한 회원에게 결제한 금액만큼 일반예치금으로 환불됩니다.<br>
          ③ 상품권 상태로 유효기간은 구매일로부터 1년이며, 만료 1개월 전 고객센터에 연락하거나 미트박스가 별도 고지한 방법을 통해 유효기간을 연장(1년 단위)할 수 있습니다. 유효기간이 만료된 경우 만료된 날로부터 1일 후 상품권을 결제한 회원에게 결제한 금액이 일반예치금으로 환불됩니다.<br>
          ④ 상품권을 사용하여 상품권 예치금으로 전환하는 경우에는 전환일로부터 5년을 유효기간으로 하며, 유효기한이 경과하는 경우 상품권 예치금은 소멸됩니다. 단, 유효기간을 별도로 정한 경우에는 별도로 정한 유효기간이 적용됩니다.<br>
          ⑤ 상품권을 사용하여 예치금으로 전환된 전체 예치금이 1만원 이하일 경우에는 100분의 80이상을, 1만원 초과일 경우에는 100분의 60이상을 사용한 경우 상품권 예치금 보유자의 의사에 따라서 환불받을 수 있습니다. 단, 상품권 예치금을 타인에게 전달 또는 선물한 예치금은 계산에서 제외됩니다.<br>
          ⑥ 본인이 고의 또는 부주의로 상품권 바코드 등을 분실하여 발생된 책임은 구매자 본인에게 있습니다.<br>
          ⑦ 회원 탈퇴시 사용하지 않은 상품권 예치금이 있을 경우 탈퇴를 진행할 수 없습니다.<br>
          ⑧ 회원이 보유할 수 있는 상품권의 합계는 최대 200만원으로, 상품권의 보유금액 합계는 아래 3가지 항목의 합계금액 입니다.<br>
          1. 상품권 구매 후 상품권 예치금으로 미 전환된 상품권 금액<br>
          2. 상품권 구매 후 타인에게 선물하였지만, 선불받은자가 아직 수락하지 않은 상품권 금액<br>
          3. 상품권 예치금으로 전환 후 미사용한 상품권 예치금 잔액(타인으로부터선물받아 미사용한 상품권 예치금 잔액포함)<br>
        </p>
        <p>제17조 (미트포인트)<br>
          ① 회사는 미트박스 사업자회원이 신규가입, 상품의 구매, 기타 프로모션에 참여하는 경우 등 회사가 규정한 지급요건에 따라 미트포인트를 제공해 줄 수 있습니다.<br>
          ② 미트포인트는 사업자회원을 대상으로 회원상태가 정상인 경우에 한하여 지급됩니다.<br>
          ③ 지급조건에 부합한 미트포인트 적립 시 소수점 첫째자리를 기준으로 반올림하여 적립되며, 1포인트는 1원의 의미를 갖습니다.<br>
          ④ 미트포인트는 단독으로 또는 다른 결제수단과 함께 미트박스에서 사용되는 지불수단으로 주문결제 및 대금지불이 가능하며, 사용금액만큼 미트포인트가 차감됩니다. 최소사용금액 제한없이 1원 단위로 사용이 가능합니다.<br>
          ⑤ 미트포인트의 부여 및 사용에 관한 상세한 사항은 회사가 정한 정책에 따르며, 회사는 미트박스 서비스의 화면 내 별도 안내페이지를 통하여 이를 이용회원에게 안내합니다.<br>
          ⑥ 미트포인트를 사용하여 구매한 상품을 주문 취소하거나 반품하는 경우에는 사용한 포인트는 반환(재적립)되며, 반환된 포인트의 유효기간의 만료일은 최초 적립 시의 유효기간의 만료일과 동일합니다.<br>
          ⑦ 무상으로 적립받은 미트포인트는 현금으로 전환하거나 환불이 불가하고, 회원 본인의 구매에 한하여 사용할 수 있으며, 일부 품목에 따라 사용이 제한될 수 있습니다.<br>
          ⑧ 다음의 각 호에 해당하는 경우 미트포인트는 자동 소멸됩니다.<br>
          1. 미트포인트의 소멸시효가 경과하는 경우<br>
          2. 미트박스 서비스의 탈퇴 등 이용계약이 해지되는 경우<br>
          ⑨ 회사가 회원에게 미트포인트를 적립하였으나, 해당 적립 원인이 매출취소, 반품, 기타의 사유에 의해 소멸되는 경우 해당 적립분만큼 잔여 포인트에서 자동차감 됩니다. 잔여 포인트가 없거나(0원) 잔여 포인트가 포인트 차감분보다 부족한 경우에는 마이너스 포인트로 처리되며, 추후 적립되는 포인트로 충당됩니다. 또한 회원 탈퇴신청 시 마이너스 포인트가 남아있는 경우, 회원은 회사에 해당 잔여액만큼 지불하지 아니하면 회원탈퇴 신청이 불가능합니다.<br>
          ⑩ 마이너스 포인트가 1만원 이상이 되는 경우, 회사는 회원에게 유선 또는 e-mail로 통보합니다.<br>
          ⑪ 회원이 법률에 위반된 행위 혹은 부정한 방법을 통해 미트포인트를 취득한 사실이 확인될 경우, 해당 포인트의 사용이 중지되거나, 회사에 의하여 회수될 수 있습니다.<br>
          ⑫ 회원이 적립받은 미트포인트는 회사에서 정한 기한내에만 이용이 가능하며, 포인트의 소멸시효는 원칙적으로 발급일로부터 1년이나, 별도의 유효기간을 정한 포인트의 경우 해당 유효기간이 적용됩니다.<br>
        </p>
        <p>제18조 (저작권 정책)<br>
          ① 회사는 서비스를 이용함에 있어 저작권자의 저작권 보호를 위한 정책을 수립하여 운영하며 회원은 회사의 저작권 정책을 준수하여야 합니다.<br>
          ② 회원이 회사가 제공하는 구매서비스를 이용하면서 작성한 각종 게시물의 저작권은 이를 작성한 회원 본인에게 있으며, 해당 게시물의 저작권 침해를 비롯한 민·형사상 모든 책임은 회원 본인이 부담합니다.<br>
          ③ 회사는 회원이 등록한 게시물을 검색 또는 다른 사이트에 노출하거나, 판촉, 홍보 및 기타의 자료로 무상 사용할 수 있으며, 회사가 제공하는 서비스 내에서 회원 게시물의 복제, 전시, 전송, 배포 및 2차적 저작물과 편집저작물 작성을 할 수 있습니다. 다만, 해당 게시물을 등록한 회원이 게시물의 삭제와 함께 사용중지를 요청한 경우에는 즉시 사용을 중지합니다.<br>
          ④ 회원은 본조 제3항에 언급된 회사의 사용권이 미트박스 서비스를 운영하는 동안에 한하여 확정적으로 유효합니다.<br>
          ⑤ 회사는 본조 제3항 이외의 방법으로 회원의 게시물을 상업적 이용하고자 하는 경우, 전화, 팩스, 전자우편 등의 방법을 통해 사전에 회원의 동의를 얻어야 합니다. 단, 회원정보에 등록된 연락처가 사실과 다르거나 회원이 회사의 연락에 응하지 않아 사전에 동의를 구하지 못한 경우, 회사는 사후에 동의 절차를 구할 수 있습니다. 회사가 본 항에 따라 회원의 게시물을 상업적으로 이용할 경우 회사는 별도의 보상제도를 운영할 수 있습니다.<br>
          ⑥ 회사가 작성한 저작물에 대한 저작권 기타 지적재산권은 회사에 귀속합니다.<br>
          ⑦ 이용자는 미트박스를 이용함으로써 얻은 정보를 회사의 사전 승낙 없이 복제, 송신, 출판, 배포, 방송 기타 방법에 의하여 이용하거나 제3자에게 이용하게 하는 경우 그에 대한 책임은 이용자 본인에게 있습니다.<br>
          ⑧ 회원은 미트박스 내에서 자신의 저작권이 침해된 경우 회사가 운영하는 신고센터 제도를 이용하여 자신의 정당한 권리를 보호받을 수 있습니다.<br>
          ⑨ 회사는 다음과 같은 경우 사전통보 없이 게시물을 삭제하거나 게시자에 대하여 특정서비스의 이용제한 또는 정지, 이용계약의 해지 등의 조치를 할 수 있습니다. 다만 게시물이 삭제되는 경우 해당 게시물과 관련된 게시물(답변글, 댓글 등)도 모두 삭제됩니다.<br>
          ...
          5. 기타 1호 내지 4호에 준하는 중대한 사유로 인하여 회사가 제공하는 미트박스 서비스의 원활한 진행을 방해하는 것으로 판단되는 경우<br>
          ⑩ 회사는 제9항에 의하여 게시물을 삭제한 경우, 해당 게시자는 회사에 이의 제기하여 구제받을 수 있습니다.<br>
        </p>
        <b>제2장 미트박스(통신판매중개) 서비스</b>
        <p>제19조 (서비스의 종류)<br>
          회사에서 회원에게 제공하는 서비스의 종류는 다음과 같습니다.<br>
          ① E-Commerce 서비스: 회사가 인터넷사이트(http://www.meatbox.co.kr) 및 미트박스 어플리케이션등 통하여 회원간에 상품의 매매가 이루어질 수 있도록 온라인 거래장소를 제공하는 서비스 및 관련 부가서비스를 말합니다.<br>
          1. 판매관련 업무지원서비스<br>
          2. 구매관련 지원서비스<br>
          3. 계약체결, 결제 지원 및 결제대금 보호 서비스<br>
          4. 상품검색정보 서비스<br>
          5. 기타 전자상거래관련 서비스<br>
          6. 미트박스 광고 및 프로모션 서비스<br>
          ② 기타 서비스 : 상품문의게시판 서비스, 상품평, 전문가 리뷰 등 기타 정보제공 서비스, 회사가 직접 또는 제휴사와 공동으로 제공하는 텔레마케팅, 이벤트 서비스를 말합니다<br>
        </p>
        <p>제20조 (대리행위 및 보증인의 부인)<br>
          ① 회사는 구매자와 판매자 간의 자유로운 상품의 거래를 위한 시스템을 운영 및 관리, 제공할 뿐이므로 구매자 또는 판매자를 대리하지 않으며, 회원 사이에 성립된 거래에 관련된 책임과 회원이 제공한 정보에 대한 책임은 해당 회원이 직접 부담하여야 합니다.<br>
          ② 회사는 미트박스를 통하여 이루어지는 구매자와 판매자 간의 거래와 관련하여 판매의사 또는 구매의사의 존부 및 진정성, 등록 상품의 품질, 완전성, 안정성, 적법성 및 타인의 권리에 대한 비침해성, 구매자 또는 판매자가 입력하는 정보 및 그 정보를 통하여 링크된 URL에 게재된 자료의 진실성 또는 적법성 등 일체에 대하여 보증하지 아니하며, 이와 관련한 일체의 위험과 책임은 해당 이용자가 전적으로 부담합니다.<br>
          ③ 회사는 구매자에게 상품을 판매하거나 판매자로부터 구매하지 않으며, 단지 구매자와 판매자간의 거래의 안전성 및 신뢰성을 증진시키는 도구만을 개발 및 제공합니다.<br>
        </p>
        <p>제21조 (구매자의 서비스 이용)<br>
          ① 구매자는 상품을 구매하기 전에 반드시 판매자가 사이트 내에 작성한 상품의 상세 내용과 거래의 조건을 정확하게 확인한 후 구매를 하여야 합니다. 구매하려는 상품의 내용과 거래의 조건을 확인하지 않고 구매하여 발생한 모든 손실, 손해는 구매자 본인에게 있습니다.<br>
          ② 구매자는 이 약관 및 회사가 서비스 화면에서 고지하는 내용을 준수하여야 하며, 약관 및 고지내용을 위반하거나 이행하지 아니하여 발생하는 모든 손실, 손해에 대하여 책임을 부담합니다.<br>
          ③ 회사는 판매자가 상품 등록한 상품의 내용과 거래조건에 대해서 어떠한 보증이나 대리를 하지 않습니다. 따라서, 구매자는 상품 구매시 스스로의 책임하에 상품을 구매하여야 합니다.<br>
          ④ 구매자는 판매자와 상품매매 절차에서 분쟁이 발생한 경우 분쟁의 해결을 위하여 성실히 임하여야 하며, 분쟁해결의 불성실로 인하여 판매자와 회사에 발생한 모든 손실, 손해를 부담하여야 합니다.<br>
          ⑤ 구매자는 상품의 구매시 결제방법을 사용함에 있어 반드시 본인 명의의 결제수단을 사용하여야 하며, 타인의 결제수단의 임의사용 등을 하여서는 안 됩니다. 타인의 결제수단을 임의 사용함으로써 발생하는 회사, 결제수단 소유자, 판매자의 손실과 손해에 대한 모든 책임은 구매자에게 있습니다.<br>
          ⑥ 매매대금의 결제와 관련하여 구매자가 입력한 정보 및 그 정보와 관련하여 발생한 책임과 불이익은 전적으로 구매자가 부담하여야 합니다.<br>
          ⑦ 상품을 주문한 후 1일이내에 매매대금을 결제하지 않을 경우 주문취소로 간주되며, 해당 주문은 자동취소됩니다.<br>
          ⑧ 회사는 구매자가 매매대금 결제 시 사용한 결제수단에 대해 정당한 사용권한을 가지고 있는지의 여부를 확인할 수 있으며, 이에 대한 확인이 완료될 때까지 거래진행을 중지하거나 해당거래를 취소할 수 있습니다.<br>
          ⑨ 회사는 구매자에게 서비스가 안전하게 제공 될 수 있도록 각종 설비와 자료를 관리해야 하며, 서비스가 제공목적에 맞게 이용되고 있는지 확인하고 구매자에게 이용목적에 위반되는 부분이 확인 될 시 사유의 소명을 요청할 수가 있습니다.<br>
          ⑩ 회사가 구매자의 서비스 이용 편의를 위해 제휴업체로부터 정보를 제공받아 게재하거나 제3자가 제공하는 방식으로 미트박스 사이트내, 어플리케이션 또는 링크 방식으로 참조용 정보나 컨텐츠를 제공하는 경우라도, 구매자는 상품 구매시 자신의 판단과 책임으로 결정하여 구매하여야 하며, 회사는 어떠한 경우에도 회원의 구매결정에 대하여 책임을 부담하지 않습니다.<br>
          ⑪ 미성년자의 구매시 법정 대리인이 해당 계약에 대하여 동의를 하지 아니하면, 미성년자 본인 또는 법정대리인이 그 계약을 취소 할 수 있습니다. 미성년자의 거래에 관한 상세한 사항은 관련 법령이 정한 바에 의합니다.<br>
        </p>
        <p>제22조 (결제대금 보호서비스의 이용)<br>
          ① 회사는 미트박스 서비스의 신뢰성을 제고하고 회원간 안전한 거래가 이루어질 수 있도록 결제대금 보호서비스를 제공합니다. 결제대금 보호서비스란, 매매계약이 체결되고 구매자가 대금결제를 하였을 경우, 구매자가 판매자로부터 해당 상품을 받았을 때 미리 정한 소정의 서비스 이용료를 공제한 후 회사가 해당 판매자에게 대금을 정산해주지만, 구매자가 판매자로부터 상품을 받지 못하였거나 받았더라도 판매자에게 반품한 후 환불을 요청하였을 때에는 구매자가 결제한 금액을 회사가 구매자에게 환불해주는 서비스를 의미합니다.<br>
          ② 결제대금 보호서비스는 미트박스에서 구매하는 모든 상품에 적용이 됩니다.<br>
          ③ 결제대금 보호서비스는 회원간에 이루어지는 상품매매의 안전성과 신뢰성을 제고하고, 상품을 인수하기 전에 대금을 결제하여야 하는 구매자를 보호하기 위한 목적에서 제공합니다.<br>
          ④ 회사의 결제대금 보호서비스의 제공이 판매자 또는 구매자를 대리하는 것이 아니며, 상품의 매매와 관련하여 판매자의 의무나 구매자의 의무를 대행하는 것이 아닙니다.<br>
          ⑤ 회사의 결제대행 서비스를 통하여 상품구매대금을 결제하는 과정에서 발생하는 송금수수료 등은 대금을 송금하는 구매자가 부담하여야 합니다.<br>
          ⑥ 결제대금 보호서비스의 제공 과정에서 결제대금의 보관과 관련하여 회사가 취득하는 이자 등에 대하여는 회사에 반환을 청구할 수 없습니다.<br>
          ⑦ 결제대금 보호서비스를 이용하는 구매자는 상품의 배송완료시 회사에 구매확정, 교환 또는 반품의 의사표시를 하여야 합니다.<br>
          ⑧ 구매자의 구매확정의 의사가 있거나 구매확정의 의사가 있는 것으로 간주하는 상황이 발생하는 경우 결제대금 보호서비스는 종료됩니다.<br>
          ⑨ 배송완료가 되었음에도 구매확정기간 내에 구매자로부터 교환 또는 반품의 의사표시가 없는 경우 회사는 구매확정의 의사가 있다고 간주합니다.<br>
          ⑩ 회사는 구매자의 구매확정의 의사가 있거나 구매확정의 의사가 있다고 간주하는 경우 [전자상거래 등에서의 소비자보호에 관한 법률]의 관련 규정에 따라 관련 대금을 상품의 판매자에게 송금할 수 있습니다.<br>
          ⑪ 결제대금 보호서비스가 종료된 이후에 청약철회, 취소, 해제, 무효 등의 사유가 발생하는 경우 구매자는 판매자와 직접 청약철회, 취소, 해제 및 결제대금 환불 등에 관한 절차를 진행하여야 합니다.<br>
        </p>
        <p>제23조 (배송 및 거래완료)<br>
          ① 배송 소요기간은 입금확인일 익일을 기산일로 하여 배송완료 되기까지의 기간을 말합니다.공휴일 및 기타 휴무일 또는 천재지변 등 불가항력적인 사유가 발생한 경우 그 해당기간은 배송 소요기간에서 제외됩니다.<br>
          ② 배송비는 판매자와 구매자의 특약으로 달리 정하지 않는 한 구매자가 부담함을 원칙으로 합니다. 다만 회사가 판매자를 대신하여 배송비를 구매자에게 직접 청구하여 구매자가 배송비를 회사에 지급한 경우에 구매자는 세금계산서 등 증빙서류의 발급을 회사에 요청할 수 있으나, 배송과 관련된 분쟁은 판매자와 구매자, 배송업체 등 관련 당사자간에 해결하여야 하며 회사는 이에 대하여 어떠한 책임도 부담하지 않습니다.<br>
          ③ 구매확정이 이루어진 이후에 발생하는 상품의 하자 등 결제대금의 환불사유가 발생하는 경우 회사는 이에 개입하지 않습니다. 구매확정 이후에 발생하는 판매자와 구매자 사이의 모든 분쟁은 거래당사자인 판매자와 구매자간에 해결하여야 합니다.<br>
          ④ 회사는 구매확정 기간 내에 구매자로부터 구매확정, 교환 또는 반품에 대한 의사표시가 없는 경우 해당 거래에 대하여 구매확정의 의사가 있는 것으로 간주하여 익영업일에 자동구매확정 처리(이하 “자동구매확정”이라 합니다)를 할 수 있습니다. 다만 구매확정 기간 내에 구매자가 실제 상품을 수령하지 못한 경우 구매자는 상품미도착 신고를 할 수 있습니다.<br>
          ⑤ 자동구매확정 처리가 완료되면, 회사는 보관중인 결제대금 중 판매자에게 지급할 상품판매대금을 판매자가 지정한 방법으로 지급할 수 있습니다.<br>
          ⑥ 자동구매확정이 이루어진 경우 구매자는 상품의 미수령, 반품 등의 사유로 회사에 대하여 이의를 제기할 수 없으며, 상품구매대금의 환불, 상품의 교환 등의 모든 문제는 판매자와 직접 해결하여야 합니다.<br>
        </p>
        <p>제24조 (반품/교환/환불/취소)<br>
          ① 구매자는 구매한 상품에 반품·교환·환불·취소등의 하고자 할 경우 수령한 상품을 임의로 사용하거나 훼손되도록 방치하여서는 아니 되며 제품의 보관 용도에 맞게 반드시 냉장 또는 냉동으로 보관해야 하며 회사에서 정하는 반품 처리 기준에 의거하여 반품 절차를 진행해야 합니다.<br>
          ② 구매자는 '전자상거래 등에서의 소비자보호에 관한 법률’ 제17조에 의해, 구매한 상품을 배송 받은 후 7일 이내에 반품 또는 교환을 요청할 수 있으며, 반품에 관한 일반적인 사항은 ' 상거래 등에서의 소비자보호에 관한 법률’등 관련 법령이 판매자가 제시한 조건보다 우선합니다. 단, 다음 각 호의 경우에는 구매자가 반품 또는 교환을 요청할 수 없습니다.<br>
          1. 구매자의 귀책사유로 말미암아 상품이 훼손된 경우<br>
          2. 구매자의 사용 또는 일부 소비에 의하여 상품의 가치가 현저히 감소한 경우<br>
          3. 시간의 경과에 의하여 재판매가 곤란할 정도로 상품의 가치가 현저히 감소한 경우<br>
          4. 복제가 가능한 상품의 포장을 훼손한 경우<br>
          5. 기타 구매자가 환불 또는 교환을 요청할 수 없는 합리적인 사유가 있는 경우<br>
          ③ 회사는 구매자의 교환 또는 반품의 의사표시가 접수되면, 즉시 그러한 사실을 판매자에게 통보합니다.<br>
          ④ 반품 또는 교환에 필요한 왕복 배송비 기타 필요한 비용은 귀책사유가 있는 쪽에서 부담합니다.<br>
          ⑤ 반품신청시 반품송장번호를 미기재할 경우 반품처리 및 환불이 지연될 수 있습니다.<br>
          ⑥ 교환신청을 하더라도 판매자에게 교환할 물품의 재고가 없는 경우 교환이 불가능하며, 이 경우 반품으로 처리됩니다.<br>
          ⑦ 교환에 소요되는 비용은 물품하자의 경우에는 판매자가 왕복배송비를 부담하나 구매자의 변심에 의한 경우에는 구매자가 부담합니다.<br>
          ⑧ 배송상의 문제로 인해 구매자가 입은 손해에 대한 책임은 책임의 소재를 발생시킨 해당 배송업체를 지정한 판매자에게 있습니다.<br>
          ⑨ 회사가 구매자의 구매대금 결제 확인에 대한 통지를 한 후 판매회원이 상당 기간 구매자의 주문 정보를 확인하지 않거나 배송 절차를 이행하지 않는 경우 회사는 구매자의 요구가 있는 경우 해당 거래를 취소할 수 있으며, 거래가 취소되면 보관중인 상품 구매대금은 구매자에게 환불됩니다. 아울러, 회사는 개별적인 정책으로 구매자의 요구 없이도 자동 환불처리 등의 조치를 취할 수 있으며 이 경우 사전에 공지합니다.<br>
          ⑩ 회사는 판매자가 구매자의 교환, 반품에 대하여 사실확인 진행절차에 적극적으로 임하지 않거나 지연하는 경우 구매자에게 교환 또는 반품의 원인을 파악한 후 해당 거래를 취소하여 구매자에게 보관중인 결제대금을 환불할 수 있습니다.<br>
          ⑪ 회사는 구매자의 교환 또는 반품의 요청이 정당하지 않다고 판단하는 경우는 구매자의 의사표시를 취소하여 보관중인 결제대금을 판매자에게 지급할 수 있습니다. 또한 구매자의 교환, 반품 등이 접수된 날로부터 14일이 경과할 때까지 기수령한 상품을 판매자에게 반환하지 않거나 연락(전화, 이메일 등)이 되지 않는 경우에는 교환, 반품의 의사를 철회한 것으로 간주하여 보관중인 결제대금을 판매자에게 지급할 수 있습니다.<br>
          ⑫ 결제대금의 환불은 구매자의 결제가 확인된 거래가 취소되는 경우 회사는 거래가 취소된 날로부터 2영업일 이내에 구매자에게 환불에 필요한 조치를 합니다. 신용카드의 결제의 경우는 즉시 결제 승인취소 처리가 되고, 미트박스 선불전자지급수단으로 결제한 거래가 취소되는 경우에는 즉시 환불조치를 합니다.<br>
          ⑬ 카드결제를 통한 구매건의 환불은 카드결제 취소를 통해서만 가능하며, 어떠한 경우에도 현금잔고로 환불되지 않습니다.<br>
          ⑭ 회원은 상품이 발송되기 전까지 구매를 취소할 수 있으며, 배송중인 경우에는 구매를 취소 할 수 없습니다.<br>
        </p>
        <p>제25조 (금지행위)<br>
          ① 회사가 제공하는 결제대금 보호 서비스를 이용하지 않고 판매자와 구매자간에 직접적으로 거래하는 행위(직거래)는 거래의 안전을 위하여 금지되고, 직거래를 통하여 발생한 제반 문제에 대한 책임은 거래 당사자에게 있으며 회사는 이에 관하여 어떠한 책임도 지지 않습니다.<br>
          ② 직거래를 하거나 이를 유도한 것으로 확인된 판매자는 서비스 이용이 정지되거나 이용계약이 해지될 수 있으며, 회원은 직거래를 하거나 유도한 판매자를 회사가 운영하는 안전거래센터에 신고할 수 있습니다.<br>
          ③ 회사가 제공하는 서비스 이용방법에 의하지 아니하고 비정상적인 방법으로 서비스를 이용하거나 시스템에 접근하는 행위는 금지됩니다.<br>
          ④ 회사는 시스템 부정행위가 확인된 경우 해당 회원에 대하여 회사가 부가적으로 제공한 혜택의 일부 또는 전부의 회수, 특정서비스의 이용제한, 이용계약의 해지 등의 조치를 할 수 있습니다.<br>
          ⑤ 타인의 명의, 카드정보, 계좌정보 등을 도용하여 회사가 제공하는 구매서비스를 이용하는 행위는 금지됩니다.<br>
          ⑥ 물품판매 또는 용역제공을 가장하여 자금을 융통하는 것과 같이 여신전문금융업법 등 법령에 의하여 금지된 방법으로 비정상적인 결제를 하는 것은 금지되며, 해당 내용이 확인된 경우 회사는 회원의 거래를 취소하거나 중지시키고 관련기관에 통보할 수 있습니다.<br>
          ⑦ 실제 상품의 배송이 없는 등 상품의 실질적인 구매의사가 없는 구매행위 및 회사가 제공하는 할인율 등을 이용한 비정상적인 거래를 하는 것은 금지되며, 해당 내용이 확인된 경우 회사는 당해 거래를 취소하고 서비스이용을 제한하거나 이용계약을 해지할 수 있습니다.<br>
        </p>
        <p>제26조 (분쟁조정 센터)<br>
          ① 회사는 회원 상호간 또는 회원과 제3자 사이의 분쟁을 조정하기 위하여 분쟁조정센터를 운영합니다.<br>
          ② 회사가 분쟁조정센터를 통하여 분쟁을 조정할 경우 회사는 제3자의 입장에서 공정하게 분쟁의 조정에 임하여야 합니다.<br>
          ③ 분쟁조정센터의 조정에 대하여 회원은 신의칙에 따라 성실히 응하여야 합니다.<br>
        </p>
        <p>제27조 (회사의 면책)<br>
          ① 판매자와 구매자간에 이루어지는 상품의 매매와 관련하여 발생하는 상품의 배송, 청약철회 또는 반품 등의 거래진행은 거래의 당사자인 판매자와 구매자 간에 수행되어야 하며, 회사는 거래진행에는 관여하지 않으며 어떠한 책임도 부담하지 않습니다.<br>
          ② 제12조 제2항의 사유로 인하여 서비스를 일시적으로 중단하는 경우 회사는 이로 인하여 이용자 또는 제3자가 입은 손해에 대하여 책임지지 않습니다.<br>
          ③ 제12조 제3항의 사유로 인하여 서비스를 제한하거나 중단하는 경우 회사는 불가항력을 이유로 그 책임을 면합니다.<br>
          ④ 회사는 이용자의 귀책사유로 인한 서비스 이용의 장애에 대하여 책임을 지지 않습니다.<br>
          ⑤ 이용자가 자신의 개인정보를 타인에게 유출 또는 제공함으로써, 발생하는 피해에 대해서 회사는 일절 책임을 지지 않습니다.<br>
          ⑥ 회사의 서비스 화면에서 링크, 배너 등을 통하여 연결된 회사(이하 피연결회사)와 이용자간에 이루어진 거래에 회사는 개입하지 않으며, 해당 거래에 대하여 책임을 지지 않습니다.<br>
          ⑦ 회사는 서비스 화면에 표시되는 판매자 또는 제3자가 제공하는 상품 또는 정보 등의 정확성, 적시성, 타당성 등에 대하여 보증 하지 않으며, 그와 관련하여 어떠한 책임도 부담하지 아니 합니다.<br>
          ⑧ 회사는 제30조 제1항에 따른 서비스나 기능의 전부 또는 일부의 수정이나 중단으로 인한 책임을 부담하지 않습니다.<br>
        </p>
        <b>제3장 미트박스 서비스</b>
        <p>제28조 (서비스 내용)<br>
          ① 미트박스 서비스는 판매회원이 미트박스 사이트에서 상품을 판매할 때에 결제 및 결제대금 보호에 필요한 시스템을 제공하는 것이며, 회원에게 상품을 판매하거나 구매하지 않습니다.<br>
          ② 미트박스 서비스를 통하여 결제가 이루어진 경우, 해당 결제 내용은 ‘마이페이지’를 통하여 확인하실 수 있습니다.<br>
          ③ 미트박스 서비스를 이용하기 위해 회사가 제공하는 양식에 따라 구매자가 직접 상품명, 결제금액 등 결제정보를 기재하여야 합니다.<br>
          ④ 미트박스 서비스를 제공하는 회사는 회원 상호간의 거래에 대한 당사자가 아니며, 해당 거래는 회원이 직접 책임을 부담합니다.<br>
        </p>
        <p>제29조 (금지행위 및 준용 등)<br>
          ① 회원이 미트박스 서비스를 이용함에 있어, 회사가 제공하는 양식에 허위의 내용을 기재하거나 회원 상호간 공모하여 관련 법령에서 금지하고 있는 물건을 거래하거나 회사가 정하는 합리적인 정책에 반하는 이용을 하는 등의 행위가 발견될 경우, 회사는 해당 결제를 취소하고 이용을 정지하거나 이용계약을 해지하는 등의 조치를 취할 수 있습니다.<br>
          ② 미트박스 서비스의 이용에 대해서는 ‘미트박스 전자금융거래 이용약관’ 및 개별약관, 정책이 적용되며, 성질에 반하지 않는 한 제2장 미트박스(통신판매중개) 서비스’의 규정이 준용됩니다.<br>
        </p>
        <b>제4장 기타사항</b>
        <p>제30조 (준거법 및 관할법원)<br>
          ① 이 약관과 회사와 회원간의 서비스 이용 계약, 회원 상호간의 물품거래에 대해서는 대한민국 법령이 적용됩니다.<br>
          ② 이 서비스와 관련하여 회사와 이용자 사이에 분쟁이 발생하여 소송이 제기되는 경우에는 서울중앙지방법원을 1심 관할법원으로 합니다.<br>
        </p>
        <p>제31조 (기타 조항)<br>
          ① 회사는 필요한 경우 특정 서비스나 기능의 전부 또는 일부를 미트박스 사이트를 통해 미리 공지한 후 일시적 또는 영구적으로 수정하거나 중단할 수 있습니다.<br>
          ② 각 당사자는 상대방의 서면동의 없이 이 약관상의 권리와 의무를 제3자에게 양도하거나 처분할 수 없습니다.<br>
          ③ 이 약관과 관련하여 당사자간의 합의에 의하여 추가로 작성된 계약서, 협정서, 통보서 등과 회사의 정책변경, 법령의 제개정 또는 공공기관의 고시나 지침 등에 의하여 회사가 미트박스 사이트를 통해 공지하는 내용도 이용계약의 일부를 구성합니다.<br>
          ④ 회사는 구매회원이 서비스 이용 시 발생 할 수 있는 정당한 의견이나 불만 사항의 적극적 수렴, 그의 해결, 회원 상호간에 분쟁 조정을 위하여 고객센터를 설치하고 운영합니다. 회사는 회원이 제기하는 각종 불만사항 및 의견에 대하여 적극적으로 검토 후 정당하다고 판단되는 경우 이를 신속하게 처리하며, 즉시 처리가 곤란한 사항에 대하여서는 그 사유와 처리기간에 대하여 e-mail 혹은 전화를 통하여 회원에게 통보를 합니다.<br>
        </p>
        <ul>
          <li>공고일자 : 2025. 09. 10</li>
          <li>시행일자 : 2025. 09. 18</li>
          <li>버전 : v1.5</li>
        </ul>
      </div><!-- //of-auto -->
      <div class="btn-group">
        <ul>
          <li class="chkeck_che" id="member-terms-pop01">
            <label class="radio" id="member-terms-pop01_agree" name="agree">동의함
              <input type="radio" name="radio">
            </label>
            <label class="radio" id="member-terms-pop01_disagree" name="disagree">동의하지 않음
              <input type="radio" name="radio">
            </label>
          </li>
          <li class="close">
...
</div><!-- //alert-pops member-terms-pop -->

<!-- 개인정보 수집 및 이용 동의(필수) -->
<div class="alert-pops member-terms-pop member-terms-pop02">
  <div class="alert">
    <div class="alert_content">
      <div class="alert-tit">
      ...
      <div class="of-auto">
        <div class="table">
          <table class="table-style04">
            ...
            <tbody>
            <tr>
              <td>본인식별 확인, 서비스 제공 및 고객 민원 처리</td>
              <td>이름, 아이디, 비밀번호, 휴대폰 번호, 이메일 주소</td>
              <td>회원 탈퇴 후 5일 이내 또는 법령에 따른 보존기간</td>
            </tr>
            </tbody>
          </table>
        </div><!-- //table -->
      ...
      <div class="btn-group">
        <ul>
          <li class="chkeck_che" id="member-terms-pop02">
            <label class="radio" id="member-terms-pop02_agree" name="agree">동의함
              <input type="radio" name="radio">
            </label>
            <label class="radio" id="member-terms-pop02_disagree" name="disagree">동의하지 않음
              <input type="radio" name="radio">
            </label>
          </li>
          <li class="close">
...
<!-- 개인정보 수집 및 이용 동의(선택) -->
<div class="alert-pops member-terms-pop member-terms-pop03">
  <div class="alert">
    <div class="alert_content">
      <div class="alert-tit">
        <h4>개인정보 수집 및 이용 동의(선택)</h4>
      </div>
      <div class="of-auto">
        <div class="table">
          <table class="table-style04">
      ...
      <div class="btn-group">
        <ul>
          <li class="chkeck_che" id="member-terms-pop03">
            <label class="radio" id="member-terms-pop03_agree" name="agree">동의함
              <input type="radio" name="radio">
            </label>
            <label class="radio" id="member-terms-pop03_disagree" name="disagree">동의하지 않음
              <input type="radio" name="radio">
            </label>
          </li>
          <li class="close">
...
<!-- 광고성 정보 수신 동의(선택) -->
<div class="alert-pops member-terms-pop member-terms-pop04">
  <div class="alert">
    <div class="alert_content">
      <div class="alert-tit">
        <h4>광고성 정보 수신 동의(선택)</h4>
      </div>
      <div class="of-auto">
        <p>미트박스에서 제공하는 이벤트/혜택 등 다양한 정보를 휴대전화(미트박스 앱 알림, 문자, 알림톡 등), 이메일로 받아보실 수 있습니다.</p>
        <p>※ 선택 약관에 대한 동의는 거부하실 수 있으며, 회원가입은 가능하지만 일부 서비스 이용과 광고, 이벤트 등의 혜택 정보 제공이 제한될 수 있습니다.</p>
      </div><!-- //of-auto -->
      <div class="btn-group">
        <ul>
          <li class="chkeck_che" id="member-terms-pop04">
            <label class="radio" id="member-terms-pop04_agree" name="agree">동의함
              <input type="radio" name="radio">
            </label>
            <label class="radio" id="member-terms-pop04_disagree" name="disagree">동의하지 않음
              <input type="radio" name="radio">
            </label>
          </li>
          <li class="close">
...
<!-- //약관동의 팝업!! --><script type="text/javascript">
  (function() {
